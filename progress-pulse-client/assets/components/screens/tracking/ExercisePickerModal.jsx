...
export default function ExercisePickerModal({ visible, onClose, day, log, onPick }) {
  return (
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
      <View className="flex-1 bg-black/60 items-center justify-center px-4">
        <View className="w-full max-w-[560px] rounded-2xl bg-card border border-border p-4">
          <View className="flex-row items-center justify-between mb-3">
            <Text className="text-primary text-lg font-extrabold">Pick exercise — {day?.name}</Text>
                ...
                return (
                  ...
                  >
                    <View className="flex-row items-center justify-between">
                      ...
                      <Text className="text-muted text-xs">planned: {ex.sets} • max {ex.lastMaxKg ?? 0}kg</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
